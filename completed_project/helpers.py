def create_todo_frame(tk, frame, x, y, bg):
    ...
    create_frame = tk.Frame(frame, width=frame_width, height=y, bg='#ffffff')
    create_frame.place(x=5, y=100)

    #   labels
    note= tk.Label(create_frame, text="Create Todo", font=('bold', 20), bg='#ffffff', fg=bg)
    note.place(x=int(frame_width/2)-80, y=2)

    todo_id = tk.Label(create_frame, text="ID", font=('bold', 10), bg='#ffffff', fg=bg)
    todo_id.place(x=20, y=50)

    title= tk.Label(create_frame, text="Title", font=('bold', 10), bg='#ffffff', fg=bg)
    title.place(x=20, y=90)

    #   inputs
    id_entry = tk.Entry(create_frame)
    ...
    message = """
            ...
            """
    warning= tk.Label(create_frame, text=message, font=('bold', 8), bg='#ffffff', fg=bg)
